  ...
  while(iteration < NUM_ITERATIONS){
    while (  !(signalled[PAPER] == 1 && signalled[TOBACCO] == 1)
          ...
          || !(signalled[MATCH] == 1 && signalled[TOBACCO] == 1)) {
            printf("signal_count[PAPER]: %d, smoke_count[PAPER]: %d \n", signal_count [PAPER], smoke_count[PAPER]);
            printf("signal_count[MATCH]: %d, smoke_count[MATCH]: %d \n", signal_count [MATCH], smoke_count[MATCH]);
