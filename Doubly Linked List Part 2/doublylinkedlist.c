...
void swap_beside(struct node* ref1, struct node* ref2);
...
void swap(struct node* ref1, struct node* ref2) {
    ...
    if(next(ref1) == ref2){
      swap_beside(ref1, ref2);
    }else if(next(ref2) == ref1){
      swap_beside(ref2, ref1);
    }else{
       struct node* prev1 = prev(ref1);         //save ref1 prev and next
       struct node* next1 = next(ref1);

       ref1->prev = prev(ref2);                //set ref1 new prev and next
       ref1->next = next(ref2);
       if(next(ref1)) ref1->next->prev = ref1; //set pointers to reft1
       if(prev(ref1)) ref1->prev->next = ref1;

       ref2->prev = prev1;                     //set ref1 new prev and next
       ref2->next = next1;
       if(next(ref2)) ref2->next->prev = ref2; //set pointers to reft2
       if(prev(ref2)) ref2->prev->next = ref2;
    }
...
void swap_beside(struct node* ref1, struct node* ref2){
  ...
  ref1->prev = ref2;
  ref2->next = ref1;
  if(next(ref1)) ref1->next->prev = ref1;
  if(prev(ref2)) ref2->prev->next = ref2;
  return;
}
...
int main() {
    ...
    shift_left(ffind(begin(list), 4), 3);
    print(list);

    printf("Shift left 1 time: ");
    shift_left(ffind(begin(list), 14), 1);
    print(list);
